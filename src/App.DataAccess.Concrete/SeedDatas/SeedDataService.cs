namespace App.DataAccess.Concrete.SeedDatas
{
    public static class SeedDataService
    {
        ...
        public static async Task SeedAsync(FlightReservationDbContext db, UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager)
        {
            await AddRolesAsync(db);
            await AddAdminAsync(db, userManager);
            await AddAppUsersAsync(db, userManager);
            await AddManufacturersAsync(db);
            await AddModelsAsync(db);
            await AddAirlinesAsync(db);
            await AddAirportsAsync(db);
            await AddRoutesAsync(db);
            await AddSchedulesAsync(db);
            await AddAircraftsAsync(db);
            await AddSeatsAsync(db);
            await AddFlightsAsync(db);
            await AddBookingsAsync(db);
        }

        // ── Roles ─────────────────────────────────────────────────────────────────

        private static async Task AddRolesAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddAdminAsync(FlightReservationDbContext db, UserManager<IdentityUser> userManager)
        {
            ...
            if (identityUser is null)
            {
                ...
                identityUser.PasswordHash = userManager.PasswordHasher.HashPassword(identityUser, AdminPassword);
            ...
            {
                db.Admins.Add(new Admin { Name = "Super", Surname = "Admin", Email = AdminEmail, IdentityId = identityUser.Id, IsSuperAdmin = true, CreatedBy = "system" });
                await db.SaveChangesAsync();
            }
        }
        ...
        private static async Task AddAppUsersAsync(FlightReservationDbContext db, UserManager<IdentityUser> userManager)
        {
        ...
        private static async Task AddManufacturersAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddModelsAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddAirlinesAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddAirportsAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddRoutesAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddSchedulesAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddAircraftsAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddSeatsAsync(FlightReservationDbContext db)
        {
        ...
        private static async Task AddFlightsAsync(FlightReservationDbContext db)
        {
                ...
                Currency cur, Guid acId, Guid alId, Guid schId, FlightStatus status)
            {
                flights.Add(new Flight
                {
                    ...
                    BaseEconomyPrice = eco,
                    BasePremiumEconomyPrice = peco,
                    BaseBusinessPrice = biz,
                    BaseFirstClassPrice = first,
                    Currency = cur,
                    FlightStatus = status,
                    AircraftId = acId,
                    AirlineId = alId,
        ...
        private static async Task AddBookingsAsync(FlightReservationDbContext db)
        {
